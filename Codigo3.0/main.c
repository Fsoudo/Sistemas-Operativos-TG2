...
void Cleanup(int sig) {
  if (gamestate) {
    // Tentar parar o jogo de forma limpa
    // Apenas o pai deve limpar idealmente, mas aqui simplificamos
    gamestate->game_running = 0;
    shmdt(gamestate);
  }

  // Apenas tentar remover se formos o dono (pai) ou no final mesmo
  // Para evitar erros de "invalid argument" se ja tiver sido removido,
  // ignoramos erro
  shmctl(shmid, IPC_RMID, NULL);
  semctl(semid, 0, IPC_RMID);

  printf("\n[SISTEMA] Recursos limpos (SHM + SEM). Adeus.\n");
  exit(0);
}
...
void MonsterLogic() {
  ...
  while (1) {
    sleep(
        5); // Movimento a cada 5 segundos (reduzi de 25 para testar mais facil)
...
void ProcessCommand(char *full_command) {
  ...
  if (strcmp(cmd, "norte") == 0) {
    ...
    P(SEM_IO);
    printf("Comandos: norte, sul, este, oeste, ver, apanhar, atacar, mapa, "
           "sair\n");
    V(SEM_IO);
